...
class Check(Target):
    ...
    def run(self):
        ...
        term.green('Linting pyliblinear sources...')
        res = analysis.pylint('pylintrc', 'pyliblinear')
        if res == 2:
...
class Compile(Target):
    ...
    def run(self):
        ...
        for name in shell.files("%s/pyliblinear" % self.dirs['lib'], '*.py'):
            self.compile(name)
        term.write("%(ERASE)s")
